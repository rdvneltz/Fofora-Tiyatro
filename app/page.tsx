...
import { ArrowRight, ChevronLeft, ChevronRight, Instagram, Mail, MapPin, Menu, MessageCircle, Pause, Play, Quote, Sparkles, Star, X } from 'lucide-react'
...
const defaultContent={nowTitle:'Şu Anda Fofora’da',nowItems:[['SIRADAKİ OYUN','Yeni sezon hazırlıkları başladı'],['KAYITLAR','Çocuk, genç ve yetişkin grupları'],['BİZDEN HABERLER','Sahnede büyüyen bir topluluk']],playsTitle:'Yaklaşan Oyunlar',calendarTitle:'Takvim',educationTitle:'Eğitimler',reelsTitle:'Sahne Akışı',newsTitle:'Bizden Haberler',teamTitle:'Ekibimiz',contactTitle:'Bize Yazın.',contactText:'Soru, fikir, iş birliği ya da eğitim bilgisi… Mesajınız doğrudan ekibimizin gelen kutusuna ulaşsın.',sloganTitle:'“Herkesin anlatacak bir hikâyesi var.”',sloganText:'Fofora Tiyatro Üsküdar’da, hayatın tam içinde.',footerTagline:'Üsküdar’da daha fazla sahne, daha fazla insan için.',whatsappText:'Merhaba, Fofora Tiyatro hakkında bilgi almak istiyorum.'}
...
export default function Home(){
  ...
  const [index,setIndex]=useState(0),[socialIndex,setSocialIndex]=useState(0),[paused,setPaused]=useState(false),[menu,setMenu]=useState(false),[sending,setSending]=useState(false),[sent,setSent]=useState(false)
  ...
  const current=slides[index]||fallbackSlides[0],media=path(current),video=/\.(mp4|webm|mov)(\?|$)/i.test(media),reels=useMemo(()=>{const ig=instagramPosts.map(p=>({id:`ig-${p.id}`,type:p.mediaType==='VIDEO'?'video':'image',url:p.mediaUrl,thumbnail:p.mediaUrl,title:p.caption?p.caption.slice(0,40):'Instagram’dan'}));return [...gallery.flatMap(a=>a.items||[]),...ig].slice(0,8)},[gallery,instagramPosts])
  ...
  return <main className="site-shell">
    <header className="topbar"><a href="#hero" className="brand"><span>Fofora</span><small>TIYATRO</small></a><nav className={menu?'nav-links open':'nav-links'}>{[['Oyunlar','oyunlar'],['Eğitimler','egitimler'],['Neler Yaptık?','neler-yaptik'],['Hakkımızda','/hakkimizda'],['Bizden Haberler','haberler'],['Ekibimiz','ekip'],['İletişim','iletisim']].map(x=><a key={x[0]} href={x[1].startsWith('/')?`${base}${x[1]}`:`#${x[1]}`} onClick={()=>setMenu(false)}>{x[0]}</a>)}<a href="https://instagram.com/foforatiyatro" target="_blank"><Instagram size={18}/></a></nav><button className="menu-button" onClick={()=>setMenu(!menu)}>{menu?<X/>:<Menu/>}</button></header>
    ...
    <section id="oyunlar" className="section dark-section"><div className="stage-tabs"><div><button className={stageTab==='plays'?'active':''} onClick={()=>setStageTab('plays')}>{copy.playsTitle}</button><button className={stageTab==='calendar'?'active':''} onClick={()=>setStageTab('calendar')}>{copy.calendarTitle}</button></div><a href="#iletisim">Tümünü gör <ArrowRight/></a></div>{stageTab==='plays'?<div className="poster-grid">{(posts.filter(p=>/oyun|etkinlik/i.test(p.category)).slice(0,4).length?posts.filter(p=>/oyun|etkinlik/i.test(p.category)).slice(0,4):samplePosts).map((p,i)=><article className={`poster p-${i}`} key={p.id}>{p.image&&<Image src={p.image} alt="" fill sizes="30vw"/>}<span>0{i+1}</span><div><small>{p.category}</small><h3>{p.title}</h3><p>{p.excerpt}</p><a className="poster-link" href={`${base}/haberler/${p.slug}`} aria-label={`${p.title} detayını aç`}><ArrowRight/></a></div></article>)}</div>:<div className="calendar-list">{(calendar.length?calendar.slice(0,4):[['PZT 14','17:30','Çocuk Tiyatro Atölyesi'],['SALI 15','19:30','Yetişkin Oyunculuk'],['CMT 19','20:00','Fiyonk — Oyun'],['PAZ 20','13:00','Genç Grup Provası']]).map((x:any)=><article key={x.id||x[0]}><strong>{x.id?new Date(x.date).toLocaleDateString('tr-TR',{weekday:'short',day:'2-digit'}).toUpperCase():x[0]}</strong><b>{x.startTime||x[1]}</b><span>{x.title||x[2]}</span></article>)}</div>}</section>
    <section id="egitimler" className="section paper-section"><Heading eyebrow="SAHNEYE ÇIK" title={copy.educationTitle} light/><div className="education-grid">{services.slice(0,5).map((s,i)=><article className="education-card" key={s.id}><div className={`education-visual e-${i}`}>{s.image&&<Image src={s.image} alt={s.title} fill sizes="20vw"/>}</div><small>{s.ageGroup}{s.duration&&` • ${s.duration}`}</small><h3>{s.title}</h3><p>{s.description}</p><a href={`${base}/egitimler/${s.id}`}><ArrowRight/></a></article>)}</div><aside className="slogan-panel"><strong>{copy.sloganTitle}</strong><span>{copy.sloganText}</span></aside></section>
    ...
    <section className="section reel-section"><Heading eyebrow="PERDENİN ARKASI" title={copy.reelsTitle} light/><div className="reel-track">{socialItems.map((r,i)=><article className={`reel-card r-${i}`} key={r.id}>{r.url&&(r.type==='video'?<video src={r.url} muted playsInline/>:<Image src={r.thumbnail||r.url} alt={r.title||''} fill sizes="220px"/>)}<span><Play fill="currentColor"/></span><h3>{r.title}</h3></article>)}</div></section>
    <section id="haberler" className="section news-section"><Heading eyebrow="GÜNCEL" title={copy.newsTitle} light/><div className="news-grid">{(posts.slice(0,3).length?posts.slice(0,3):sampleNews).map((p,i)=><article key={p.id}><div className={`news-image n-${i}`}>{p.image&&<Image src={p.image} alt={p.title} fill sizes="33vw"/>}</div><small>{p.category} • {new Date(p.createdAt).toLocaleDateString('tr-TR')}</small><h3>{p.title}</h3><p>{p.excerpt}</p><a href={`${base}/haberler/${p.slug}`}>Devamını oku <ArrowRight/></a></article>)}</div></section>
    {team.length>0&&<section id="ekip" className="section team-section"><Heading eyebrow="BİRLİKTE ÜRETİYORUZ" title={copy.teamTitle}/><div className="team-grid">{team.slice(0,4).map(m=><article key={m.id}><div><Image src={m.image} alt={m.name} fill sizes="25vw"/></div><h3>{m.name}</h3><p>{m.title}</p></article>)}</div></section>}
    <section id="iletisim" className={testimonials.length?'contact-section has-testimonials':'contact-section'}>
      {testimonials.length>0&&<aside className="contact-testimonials"><h3><Quote/> Ne Diyorlar?</h3><div className="testimonial-stack">{testimonials.slice(0,3).map(t=><button key={t.id} className="testimonial-card" onClick={()=>setTestimonialsOpen(true)}><p>“{t.content}”</p><span>{t.name}{t.title?` · ${t.title}`:''}</span></button>)}</div><button className="testimonial-more" onClick={()=>setTestimonialsOpen(true)}>Tümünü gör <ArrowRight/></button></aside>}
      ...
      <form className="contact-teaser" onSubmit={e=>{e.preventDefault();setMessageOpen(true)}} onClick={()=>setMessageOpen(true)}><div className="form-row"><label className="teaser-name">Adınız Soyadınız<input readOnly tabIndex={-1}/></label><label className="teaser-email">E-posta<input readOnly tabIndex={-1}/></label></div><label className="teaser-subject">Konu<select tabIndex={-1} defaultValue=""><option value="">Bir konu seçin</option></select></label><label className="teaser-message">Mesajınız<textarea readOnly tabIndex={-1}/></label><button className="button form-button">Mesaj yaz <ArrowRight/></button></form>
    </section>
    {testimonialsOpen&&<div className="testimonials-modal" role="dialog" aria-modal="true" aria-label="Tüm yorumlar"><button className="modal-close" onClick={()=>setTestimonialsOpen(false)} aria-label="Kapat"><X/></button><div className="testimonials-modal-inner"><p className="eyebrow ink">NE DİYORLAR?</p><h2>Yorumlar</h2><div className="testimonials-grid">{testimonials.map(t=><article key={t.id}>{t.rating>0&&<div className="stars">{Array.from({length:5},(_,i)=><Star key={i} fill={i<t.rating?'currentColor':'none'}/>)}</div>}<p>“{t.content}”</p><span>{t.name}{t.title?` · ${t.title}`:''}</span></article>)}</div></div></div>}
    <footer><a className="brand"><span>Fofora</span><small>TIYATRO</small></a><p>{copy.footerTagline}</p><div><a href="https://instagram.com/foforatiyatro"><Instagram/></a><a href={`mailto:${contact.email}`}><Mail/></a></div><small>© {new Date().getFullYear()} Fofora Tiyatro</small></footer>
