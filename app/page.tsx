...
type Team = { id:string; name:string; title:string; image:string; imagePosX?:number; imagePosY?:number }
...
export default function Home(){
  ...
  const stageTabAutoRef=useRef(true)
  ...
  useEffect(()=>{const t=setInterval(()=>{if(stageTabAutoRef.current)setStageTab(v=>v==='plays'?'calendar':'plays')},10000);return()=>clearInterval(t)},[])
  const setStageTabManual=(v:'plays'|'calendar')=>{stageTabAutoRef.current=false;setStageTab(v)}
  ...
  // A link without "/" or "http(s)://" (e.g. "www.youtube.com" typed without a protocol) is
  // neither a valid site-relative path nor a URL the browser can navigate to as-is - treat it as
  // external. Internal paths get the /yeni prefix carried over, matching every other internal link.
  const resolveHref=(url:string)=>{
    const t=(url||'').trim()
    if(!t)return t
    if(/^https?:\/\//i.test(t))return t
    if(t.startsWith('/'))return base&&!t.startsWith('/yeni')?`${base}${t}`:t
    return `https://${t}`
  }
  const handleStripClick=(item:StripItem)=>{
    ...
    // The homepage is a non-scrolling single screen on desktop, so this navigates to one of the
    // site's dedicated pages rather than scrolling to a section within it.
    if(item.clickAction==='section'&&item.sectionId){location.href=resolveHref(item.sectionId);return}
    if(item.clickAction==='link'&&item.linkUrl){
      const href=resolveHref(item.linkUrl)
      if(/^https?:\/\//i.test(href))open(href,'_blank')
  ...
  const calendarOverflow=calendar.length>4?calendar.length-3:0
  const calendarVisible=calendarOverflow>0?calendar.slice(0,3):calendar.slice(0,4)
