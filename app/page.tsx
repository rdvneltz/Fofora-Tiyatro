...
export default function Home(){
  ...
  return <main className="site-shell">
    ...
    <section id="hero" className="hero-stage"><AnimatePresence mode="wait"><motion.div key={current.id} className="hero-media" initial={{opacity:0,scale:1.04}} animate={{opacity:1,scale:1}} exit={{opacity:0}}>{media?(video?<video src={media} autoPlay muted playsInline loop/>:<Image src={media} alt="" fill priority sizes="100vw"/>):<div className={`hero-placeholder h-${index%2}`}/>}</motion.div></AnimatePresence><div className="hero-scrim"/><AnimatePresence mode="wait"><motion.div key={'c'+current.id} className="hero-copy" initial={{opacity:0,y:30}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-20}} onClick={()=>go(current.actionType,current.actionValue)}><p className="eyebrow">{current.subtitle}</p><h1>{current.title?.split('\n').map(x=><span key={x}>{x}</span>)}</h1><p>{current.description}</p><div className="hero-actions" onClick={e=>e.stopPropagation()}><button className="button acid" onClick={()=>go(current.actionType,current.actionValue)}>{current.actionLabel||'Keşfet'} <ArrowRight/></button>{current.secondaryActionLabel&&<button className="button outline" onClick={()=>go(current.secondaryActionType,current.secondaryActionValue)}>{current.secondaryActionLabel} <ArrowRight/></button>}</div></motion.div></AnimatePresence>
      ...
      <a className="hero-whatsapp" href={`https://wa.me/${contact.phone.replace(/\D/g,'').replace(/^0/,'90')}?text=${encodeURIComponent('Merhaba, Fofora Tiyatro hakkında bilgi almak istiyorum.')}`} target="_blank" rel="noopener noreferrer" aria-label="Fofora Tiyatro’ya WhatsApp’tan yaz"><MessageCircle/><span>WhatsApp’tan<br/>yaz</span></a>
    ...
    <section id="oyunlar" className="section dark-section"><div className="stage-tabs"><div><button className={stageTab==='plays'?'active':''} onClick={()=>setStageTab('plays')}>Yaklaşan Oyunlar</button><button className={stageTab==='calendar'?'active':''} onClick={()=>setStageTab('calendar')}>Takvim</button></div><a href="#iletisim">Tümünü gör <ArrowRight/></a></div>{stageTab==='plays'?<div className="poster-grid">{(posts.filter(p=>/oyun|etkinlik/i.test(p.category)).slice(0,4).length?posts.filter(p=>/oyun|etkinlik/i.test(p.category)).slice(0,4):samplePosts).map((p,i)=><article className={`poster p-${i}`} key={p.id}>{p.image&&<Image src={p.image} alt="" fill sizes="30vw"/>}<span>0{i+1}</span><div><small>{p.category}</small><h3>{p.title}</h3><p>{p.excerpt}</p><button><ArrowRight/></button></div></article>)}</div>:<div className="calendar-list">{[['PZT 14','17:30','Çocuk Tiyatro Atölyesi'],['SALI 15','19:30','Yetişkin Oyunculuk'],['CMT 19','20:00','Fiyonk — Oyun'],['PAZ 20','13:00','Genç Grup Provası']].map(x=><article key={x[0]}><strong>{x[0]}</strong><b>{x[1]}</b><span>{x[2]}</span></article>)}</div>}</section>
    ...
    <section id="iletisim" className="contact-section"><div className="contact-copy"><p className="eyebrow ink">BİR MERHABA YETER</p><h2>Bize<br/>Yazın.</h2><p>Soru, fikir, iş birliği ya da eğitim bilgisi… Mesajınız doğrudan ekibimizin gelen kutusuna ulaşsın.</p><div><a href={`https://wa.me/${contact.phone.replace(/\D/g,'')}`} target="_blank"><MessageCircle/> WhatsApp’tan yaz</a><a href={`mailto:${contact.email}`}><Mail/> {contact.email}</a><span><MapPin/> {contact.address}</span></div></div><form className="contact-teaser" onSubmit={e=>{e.preventDefault();setMessageOpen(true)}} onClick={()=>setMessageOpen(true)}><div className="form-row"><label className="teaser-name">Adınız Soyadınız<input readOnly tabIndex={-1}/></label><label className="teaser-email">E-posta<input readOnly tabIndex={-1}/></label></div><label className="teaser-subject">Konu<select tabIndex={-1} defaultValue=""><option value="">Bir konu seçin</option></select></label><label className="teaser-message">Mesajınız<textarea readOnly tabIndex={-1}/></label><button className="button form-button">Mesaj yaz <ArrowRight/></button></form></section>
